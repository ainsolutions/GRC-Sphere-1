...
interface DORARemediationTrackerProps {
  assessmentId?: number
}

export function DORARemediationTracker({ assessmentId }: DORARemediationTrackerProps) {
  const [remediations, setRemediations] = useState<DORARemediation[]>([])
  const [filteredRemediations, setFilteredRemediations] = useState<DORARemediation[]>([])
  const [selectedRemediation, setSelectedRemediation] = useState<DORARemediation | null>(null)
  ...
  const fetchRemediations = async () => {
    setLoading(true)
    try {
      const url = assessmentId ? `/api/dora-remediation?assessment_id=${assessmentId}` : "/api/dora-remediation"
      ...
      console.error("Failed to fetch DORA remediations:", error)
      toast({
        title: "Error",
        description: "Failed to load DORA remediation items",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const createRemediation = async () => {
    try {
      const response = await fetch("/api/dora-remediation", {
        method: "POST",
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: "DORA remediation item created successfully",
        })
      ...
      toast({
        title: "Error",
        description: "Failed to create DORA remediation item",
        variant: "destructive",
      })
    }
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold bg-gradient-to-r from-indigo-600 via-blue-600 to-purple-600 bg-clip-text text-transparent">
            DORA Remediation Tracker
          </h2>
          <p className="text-muted-foreground">Track and manage remediation actions for DORA compliance findings</p>
        </div>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      placeholder="e.g., ICT-RM-01, ICT-IM-02"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="cost_estimate">Cost Estimate (EUR)</Label>
                    <Input
                      id="cost_estimate"
                      type="number"
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-4">
            <div className="flex items-center space-x-2 flex-1">
              ...
              <Input
                ...
                className="border-indigo-200 focus:border-blue-400"
              />
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2 text-sm">
                    ...
                    <div>
                      <strong>Cost Estimate:</strong> EUR {selectedRemediation.cost_estimate.toLocaleString()}
                    </div>
                  </CardContent>
                </Card>
              </div>
