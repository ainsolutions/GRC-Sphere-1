...
export function SAMAGapAnalysis() {
  const [gapAnalysis, setGapAnalysis] = useState<SAMAGapAnalysis[]>([])
  const [filteredGaps, setFilteredGaps] = useState<SAMAGapAnalysis[]>([])
  ...
  const fetchGapAnalysis = async () => {
    setLoading(true)
    try {
      const response = await fetch("/api/sama-gap-analysis")
      if (response.ok) {
        const data = await response.json()
        setGapAnalysis(data)
      }
    } catch (error) {
      console.error("Failed to fetch SAMA gap analysis:", error)
      toast({
        title: "Error",
        description: "Failed to load SAMA gap analysis",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Identify and prioritize gaps in SAMA cybersecurity control implementation
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="border-border bg-card">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">SAR {totalCost.toLocaleString()}</div>
      ...
      <Card className="gradient-card-primary">
        <CardHeader>
          ...
          <CardDescription>Prioritized list of cybersecurity control implementation gaps</CardDescription>
        </CardHeader>
        <CardContent>
          {/* Filters */}
      ...
      <Card className="gradient-card-secondary">
        <CardHeader>
          ...
          <CardDescription>Summary of gaps across SAMA cybersecurity domains</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <Card key={domain} className="border-purple-200/50">
                  ...
                  <CardContent className="space-y-2">
                    ...
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-medium">SAR {domainCost.toLocaleString()}</span>
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
