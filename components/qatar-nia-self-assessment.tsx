...
interface QatarNIASelfAssessmentControl {
  id: number
  domain: string
  control_id: string
  control_name: string
...
export function QatarNIASelfAssessment() {
  const [controls, setControls] = useState<QatarNIASelfAssessmentControl[]>([])
  ...
  const fetchSelfAssessmentControls = async () => {
    setLoading(true)
    try {
      const response = await fetch("/api/qatar-nia-self-assessment/controls")
      if (response.ok) {
        const data = await response.json()
        setControls(data)
      }
    } catch (error) {
      console.error("Failed to fetch Qatar NIA self-assessment controls:", error)
      toast({
        title: "Error",
        description: "Failed to load Qatar NIA self-assessment controls",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const updateControl = async (controlId: number, updates: Partial<QatarNIASelfAssessmentControl>) => {
    setSaving(true)
    try {
      const response = await fetch("/api/qatar-nia-self-assessment/controls", {
        method: "PUT",
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold bg-gradient-to-r from-teal-600 via-blue-600 to-purple-600 bg-clip-text text-transparent">
            Qatar NIA Self-Assessment
          </h2>
          <p className="text-muted-foreground">
            Conduct internal assessment of Qatar NIA security controls implementation
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="border-border bg-card">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-teal-600">{compliancePercentage}%</div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="bg-gradient-to-r from-teal-600 to-blue-600 bg-clip-text text-transparent">
                Qatar NIA Security Controls Assessment
              </CardTitle>
              <CardDescription>Assess implementation status of Qatar NIA security controls</CardDescription>
            </div>
          ...
          {loading ? (
            ...
            <Tabs defaultValue="assessment" className="w-full">
              <TabsList className="grid w-full grid-cols-2">
                <TabsTrigger value="assessment">Security Controls Assessment</TabsTrigger>
              ...
              <TabsContent value="assessment" className="space-y-4">
                <div className="space-y-4">
                  {filteredControls.map((control) => (
                    <Card key={control.id} className="border-teal-200/50">
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <Textarea
                            ...
                            placeholder="Describe evidence of security control implementation..."
                            rows={2}
                          />
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            <Textarea
                              ...
                              placeholder="Who is responsible for this security control?"
                              rows={1}
                            />
                          </div>
                          <div className="space-y-2">
