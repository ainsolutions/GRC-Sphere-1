...
interface QatarNIARemediationTrackerProps {
  assessmentId?: number
}

export function QatarNIARemediationTracker({ assessmentId }: QatarNIARemediationTrackerProps) {
  const [remediations, setRemediations] = useState<QatarNIARemediation[]>([])
  const [filteredRemediations, setFilteredRemediations] = useState<QatarNIARemediation[]>([])
  const [selectedRemediation, setSelectedRemediation] = useState<QatarNIARemediation | null>(null)
  ...
  const fetchRemediations = async () => {
    ...
    try {
      const url = assessmentId
        ? `/api/qatar-nia-remediation?assessment_id=${assessmentId}`
      ...
      console.error("Failed to fetch Qatar NIA remediations:", error)
      toast({
        title: "Error",
        description: "Failed to load Qatar NIA remediation items",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const createRemediation = async () => {
    try {
      const response = await fetch("/api/qatar-nia-remediation", {
        method: "POST",
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: "Qatar NIA remediation item created successfully",
        })
      ...
      toast({
        title: "Error",
        description: "Failed to create Qatar NIA remediation item",
        variant: "destructive",
      })
    }
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold bg-gradient-to-r from-teal-600 via-blue-600 to-purple-600 bg-clip-text text-transparent">
            Qatar NIA Remediation Tracker
          </h2>
          <p className="text-muted-foreground">
            Track and manage remediation actions for Qatar NIA compliance findings
          </p>
        </div>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle className="bg-gradient-to-r from-teal-600 to-blue-600 bg-clip-text text-transparent">
                  Add New Qatar NIA Remediation Item
                </DialogTitle>
              </DialogHeader>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="cost_estimate">Cost Estimate (QAR)</Label>
                    <Input
                      id="cost_estimate"
                      type="number"
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-4">
            <div className="flex items-center space-x-2 flex-1">
              ...
              <Input
                ...
                className="border-teal-200 focus:border-blue-400"
              />
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2 text-sm">
                    ...
                    <div>
                      <strong>Cost Estimate:</strong> QAR {selectedRemediation.cost_estimate.toLocaleString()}
                    </div>
                  </CardContent>
                </Card>
              </div>
