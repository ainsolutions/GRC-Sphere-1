...
export function QatarNIAComplianceAssessment() {
  const [assessments, setAssessments] = useState<QatarNIAAssessment[]>([])
  const [requirements, setRequirements] = useState<QatarNIARequirement[]>([])
  const [selectedAssessment, setSelectedAssessment] = useState<QatarNIAAssessment | null>(null)
  ...
  useEffect(() => {
    fetchAssessments()
    fetchQatarNIARequirements()
  }, [])

  const fetchAssessments = async () => {
    setLoading(true)
    try {
      const response = await fetch("/api/qatar-nia-assessments")
      if (response.ok) {
        const data = await response.json()
        setAssessments(data)
      }
    } catch (error) {
      console.error("Failed to fetch Qatar NIA assessments:", error)
      toast({
        title: "Error",
        description: "Failed to load Qatar NIA assessments",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const fetchQatarNIARequirements = async () => {
    try {
      const response = await fetch("/api/qatar-nia/requirements")
      if (response.ok) {
        const data = await response.json()
        setRequirements(data)
      }
    } catch (error) {
      console.error("Failed to fetch Qatar NIA requirements:", error)
    }
  }
  ...
  const createAssessment = async () => {
    try {
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: "Qatar NIA assessment created successfully",
        })
      ...
      toast({
        title: "Error",
        description: "Failed to create Qatar NIA assessment",
        variant: "destructive",
      })
    }
  }
  ...
  const getDomainIcon = (domain: string) => {
    switch (domain) {
      case "Information Security Governance":
      ...
      case "System Acquisition and Development":
  ...
  const filteredAssessments = assessments.filter(
    (assessment) =>
      ...
      assessment.sector.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold bg-gradient-to-r from-teal-600 via-blue-600 to-purple-600 bg-clip-text text-transparent">
            Qatar NIA Compliance Assessment
          </h2>
          <p className="text-muted-foreground">National Information Assurance compliance for Qatar entities</p>
        </div>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle className="bg-gradient-to-r from-teal-600 to-blue-600 bg-clip-text text-transparent">
                  Create New Qatar NIA Assessment
                </DialogTitle>
              </DialogHeader>
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      value={newAssessment.sector}
                      onValueChange={(value) => setNewAssessment((prev) => ({ ...prev, sector: value }))}
                    >
                      <SelectTrigger>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Describe the scope of this Qatar NIA assessment"
                    rows={3}
                  />
                </div>
                ...
                <div className="flex justify-end space-x-2">
                  ...
                  <Button
                    ...
                    disabled={!newAssessment.assessment_name || !newAssessment.entity_type || !newAssessment.sector}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="border-border bg-card">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium bg-gradient-to-r from-teal-600 to-blue-600 bg-clip-text text-transparent">
              Total Assessments
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-teal-700">{assessments.length}</div>
          </CardContent>
        </Card>
        <Card className="border-border bg-card">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium bg-gradient-to-r from-teal-600 to-green-600 bg-clip-text text-transparent">
              Authority Notified
            </CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Tabs defaultValue="assessments" className="w-full">
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="controls">NIA Controls</TabsTrigger>
          ...
          <Card className="gradient-card-primary">
            <CardHeader>
              <CardTitle className="bg-gradient-to-r from-teal-600 to-blue-600 bg-clip-text text-transparent">
                Qatar NIA Assessments
              </CardTitle>
              <CardDescription>
                Manage and track Qatar National Information Assurance compliance assessments
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center space-x-2 mb-4">
                ...
                <Input
                  ...
                  className="max-w-sm border-teal-200 focus:border-blue-400"
                />
              </div>
              ...
              {loading ? (
                ...
                <div className="rounded-lg overflow-hidden border border-teal-200/50">
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-gradient-to-r from-teal-100/50 via-blue-100/50 to-purple-100/50">
                      ...
                      {filteredAssessments.length === 0 ? (
                        <TableRow>
                          <TableCell colSpan={10} className="text-center py-8 text-muted-foreground">
                            No Qatar NIA assessments found. Create your first assessment to get started.
                          </TableCell>
                        </TableRow>
                      ) : (
                        filteredAssessments.map((assessment) => (
                          <TableRow
                            key={assessment.id}
                            className="hover:bg-gradient-to-r hover:from-teal-50/30 hover:via-blue-50/30 hover:to-purple-50/30"
                          >
        ...
        <TabsContent value="controls" className="space-y-4">
          <Card className="gradient-card-primary">
            <CardHeader>
              <CardTitle className="bg-gradient-to-r from-teal-600 to-blue-600 bg-clip-text text-transparent">
                Qatar NIA Security Controls
              </CardTitle>
              <CardDescription>
                Comprehensive security controls framework under Qatar National Information Assurance
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {[
                  ...
                  <Card key={domain} className="border-teal-200/50">
        ...
        <TabsContent value="remediation" className="space-y-4">
          <QatarNIARemediationTracker assessmentId={selectedAssessment?.id} />
        </TabsContent>
      </Tabs>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Card>
                  ...
                  <CardContent className="space-y-2 text-sm">
                    ...
                    <div>
                      <strong>Sector:</strong> {selectedAssessment.sector}
                    </div>
                    <div>
