...
export function DORAGapAnalysis() {
  const [gapAnalysis, setGapAnalysis] = useState<DORAGapAnalysis[]>([])
  const [filteredGaps, setFilteredGaps] = useState<DORAGapAnalysis[]>([])
  ...
  const fetchGapAnalysis = async () => {
    setLoading(true)
    try {
      const response = await fetch("/api/dora-gap-analysis")
      if (response.ok) {
        const data = await response.json()
        setGapAnalysis(data)
      }
    } catch (error) {
      console.error("Failed to fetch DORA gap analysis:", error)
      toast({
        title: "Error",
        description: "Failed to load DORA gap analysis",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Identify and prioritize gaps in DORA requirements implementation</p>
        </div>
        <Button
          variant="outline"
      ...
      <Card className="gradient-card-primary">
        <CardHeader>
          <CardTitle className="bg-gradient-to-r from-indigo-600 to-blue-600 bg-clip-text text-transparent">
            DORA Requirement Gaps
          </CardTitle>
          <CardDescription>Prioritized list of DORA requirement implementation gaps</CardDescription>
        </CardHeader>
        <CardContent>
          {/* Filters */}
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="flex items-center space-x-2 flex-1">
              ...
              <Input
                placeholder="Search by requirement name, ID, or responsible party..."
                value={searchTerm}
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-gradient-to-r from-indigo-100/50 via-blue-100/50 to-purple-100/50">
                    ...
                    <TableHead>DORA Requirement</TableHead>
      ...
      <Card className="gradient-card-secondary">
        <CardHeader>
          ...
          <CardDescription>Summary of gaps across DORA domains</CardDescription>
        </CardHeader>
        <CardContent>
