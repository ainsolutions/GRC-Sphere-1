...
interface NIS2RemediationTrackerProps {
  assessmentId?: number
}

export function NIS2RemediationTracker({ assessmentId }: NIS2RemediationTrackerProps) {
  const [remediations, setRemediations] = useState<NIS2Remediation[]>([])
  const [filteredRemediations, setFilteredRemediations] = useState<NIS2Remediation[]>([])
  const [selectedRemediation, setSelectedRemediation] = useState<NIS2Remediation | null>(null)
  ...
  const fetchRemediations = async () => {
    setLoading(true)
    try {
      const url = assessmentId ? `/api/nis2-remediation?assessment_id=${assessmentId}` : "/api/nis2-remediation"
      ...
      console.error("Failed to fetch NIS2 remediations:", error)
      toast({
        title: "Error",
        description: "Failed to load NIS2 remediation items",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const createRemediation = async () => {
    try {
      const response = await fetch("/api/nis2-remediation", {
        method: "POST",
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: "NIS2 remediation item created successfully",
        })
      ...
      toast({
        title: "Error",
        description: "Failed to create NIS2 remediation item",
        variant: "destructive",
      })
    }
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold bg-gradient-to-r from-green-600 via-blue-600 to-purple-600 bg-clip-text text-transparent">
            NIS2 Remediation Tracker
          </h2>
          <p className="text-muted-foreground">Track and manage remediation actions for NIS2 compliance findings</p>
        </div>
