...
interface DORASelfAssessmentControl {
  id: number
  domain: string
  control_id: string
  control_name: string
...
export function DORASelfAssessment() {
  const [controls, setControls] = useState<DORASelfAssessmentControl[]>([])
  ...
  const fetchSelfAssessmentControls = async () => {
    setLoading(true)
    try {
      const response = await fetch("/api/dora-self-assessment/controls")
      if (response.ok) {
        const data = await response.json()
        setControls(data)
      }
    } catch (error) {
      console.error("Failed to fetch DORA self-assessment controls:", error)
      toast({
        title: "Error",
        description: "Failed to load DORA self-assessment controls",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const updateControl = async (controlId: number, updates: Partial<DORASelfAssessmentControl>) => {
    setSaving(true)
    try {
      const response = await fetch("/api/dora-self-assessment/controls", {
        method: "PUT",
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Conduct internal assessment of DORA requirements implementation</p>
        </div>
        <Button
          variant="outline"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="border-border bg-card">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-indigo-600">{compliancePercentage}%</div>
        ...
        <Card className="border-border bg-card">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Fully compliant requirements</p>
          </CardContent>
        </Card>
        <Card className="border-border bg-card">
          <CardHeader className="pb-2">
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="bg-gradient-to-r from-indigo-600 to-blue-600 bg-clip-text text-transparent">
                DORA Requirements Assessment
              </CardTitle>
              <CardDescription>Assess implementation status of DORA requirements</CardDescription>
            </div>
          ...
          {loading ? (
            ...
            <Tabs defaultValue="assessment" className="w-full">
              <TabsList className="grid w-full grid-cols-2">
                <TabsTrigger value="assessment">Requirements Assessment</TabsTrigger>
              ...
              <TabsContent value="assessment" className="space-y-4">
                <div className="space-y-4">
                  {filteredControls.map((control) => (
                    <Card key={control.id} className="border-indigo-200/50">
                      ...
                      <CardContent className="space-y-4">
                        ...
                        <div className="space-y-2">
                          ...
                          <Textarea
                            ...
                            placeholder="Describe evidence of requirement implementation..."
                            rows={2}
                          />
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="space-y-2">
                            ...
                            <Textarea
                              ...
                              placeholder="Who is responsible for this requirement?"
                              rows={1}
                            />
                          </div>
                          <div className="space-y-2">
