...
export function DORAComplianceAssessment() {
  const [assessments, setAssessments] = useState<DORAAssessment[]>([])
  const [requirements, setRequirements] = useState<DORARequirement[]>([])
  const [selectedAssessment, setSelectedAssessment] = useState<DORAAssessment | null>(null)
  ...
  useEffect(() => {
    fetchAssessments()
    fetchDORARequirements()
  }, [])

  const fetchAssessments = async () => {
    setLoading(true)
    try {
      const response = await fetch("/api/dora-assessments")
      if (response.ok) {
        const data = await response.json()
        setAssessments(data)
      }
    } catch (error) {
      console.error("Failed to fetch DORA assessments:", error)
      toast({
        title: "Error",
        description: "Failed to load DORA assessments",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  const fetchDORARequirements = async () => {
    try {
      const response = await fetch("/api/dora/requirements")
      if (response.ok) {
        const data = await response.json()
        setRequirements(data)
      }
    } catch (error) {
      console.error("Failed to fetch DORA requirements:", error)
    }
  }
  ...
  const createAssessment = async () => {
    try {
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: "DORA assessment created successfully",
        })
      ...
      toast({
        title: "Error",
        description: "Failed to create DORA assessment",
        variant: "destructive",
      })
    }
  }
  ...
  const getDomainIcon = (domain: string) => {
    switch (domain) {
      ...
      case "ICT-related Incident Management":
        return <AlertTriangle className="h-4 w-4" />
      case "Digital Operational Resilience Testing":
        return <Zap className="h-4 w-4" />
      case "Managing ICT Third-party Risk":
        return <Users className="h-4 w-4" />
      case "Information and Intelligence Sharing":
        return <Network className="h-4 w-4" />
      case "ICT Governance and Strategy":
        return <Building className="h-4 w-4" />
      case "ICT Business Continuity":
        return <RefreshCw className="h-4 w-4" />
      case "ICT Security Controls":
        return <Lock className="h-4 w-4" />
      case "ICT Asset Management":
  ...
  const entityTypes = [
    ...
    { value: "insurance-undertaking", label: "Insurance Undertaking" },
    { value: "reinsurance-undertaking", label: "Reinsurance Undertaking" },
    { value: "payment-institution", label: "Payment Institution" },
    { value: "electronic-money-institution", label: "Electronic Money Institution" },
    { value: "central-counterparty", label: "Central Counterparty" },
    { value: "trade-repository", label: "Trade Repository" },
    { value: "crypto-asset-service-provider", label: "Crypto-asset Service Provider" },
    { value: "crowdfunding-service-provider", label: "Crowdfunding Service Provider" },
  ]
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Digital Operational Resilience Act compliance for EU financial entities
          </p>
        </div>
        <div className="flex space-x-2">
          ...
          <Dialog open={isNewAssessmentOpen} onOpenChange={setIsNewAssessmentOpen}>
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle className="bg-gradient-to-r from-indigo-600 to-blue-600 bg-clip-text text-transparent">
                  Create New DORA Assessment
                </DialogTitle>
              </DialogHeader>
              <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="Supervisory Assessment">Supervisory Assessment</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Describe the scope of this DORA assessment"
                    rows={3}
                  />
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="border-border bg-card">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium bg-gradient-to-r from-indigo-600 to-blue-600 bg-clip-text text-transparent">
              Total Assessments
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-indigo-700">{assessments.length}</div>
          </CardContent>
        </Card>
        <Card className="border-border bg-card">
          <CardHeader className="pb-2">
      ...
      <Tabs defaultValue="assessments" className="w-full">
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="controls">DORA Controls</TabsTrigger>
        ...
        <TabsContent value="assessments" className="space-y-4">
          <Card className="gradient-card-primary">
            <CardHeader>
              ...
              <CardDescription>Manage and track DORA compliance assessments</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center space-x-2 mb-4">
                ...
                <Input
                  ...
                  className="max-w-sm border-indigo-200 focus:border-blue-400"
                />
              </div>
              ...
              {loading ? (
                ...
                <div className="rounded-lg overflow-hidden border border-indigo-200/50">
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-gradient-to-r from-indigo-100/50 via-blue-100/50 to-purple-100/50">
                      ...
                      {filteredAssessments.length === 0 ? (
                        <TableRow>
                          <TableCell colSpan={10} className="text-center py-8 text-muted-foreground">
                            No DORA assessments found. Create your first assessment to get started.
                          </TableCell>
                        </TableRow>
                      ) : (
                        filteredAssessments.map((assessment) => (
                          <TableRow
                            key={assessment.id}
                            className="hover:bg-gradient-to-r hover:from-indigo-50/30 hover:via-blue-50/30 hover:to-purple-50/30"
                          >
        ...
        <TabsContent value="controls" className="space-y-4">
          <Card className="gradient-card-primary">
            <CardHeader>
              ...
              <CardDescription>Digital operational resilience requirements framework</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {[
                  ...
                  <Card key={domain} className="border-indigo-200/50">
                    <CardHeader className="pb-3">
                      <CardTitle className="text-lg flex items-center space-x-2">
                        {getDomainIcon(domain)}
                        <span>{domain}</span>
                        <Badge variant="outline" className="ml-auto">
                          {requirements.filter((r) => r.domain === domain).length} requirements
                        </Badge>
                      </CardTitle>
                    </CardHeader>
                    <CardContent>
                        ...
                        {requirements.filter((req) => req.domain === domain).length > 3 && (
                          <div className="text-center pt-2">
                            <Button variant="outline" size="sm" className="text-xs bg-transparent">
                              View all {requirements.filter((req) => req.domain === domain).length} requirements
                            </Button>
                          </div>
                        )}
                      </div>
        ...
        <TabsContent value="remediation" className="space-y-4">
          <DORARemediationTracker assessmentId={selectedAssessment?.id} />
        </TabsContent>
      </Tabs>
