...
export function QatarNIAGapAnalysis() {
  const [gapAnalysis, setGapAnalysis] = useState<QatarNIAGapAnalysis[]>([])
  const [filteredGaps, setFilteredGaps] = useState<QatarNIAGapAnalysis[]>([])
  ...
  const fetchGapAnalysis = async () => {
    setLoading(true)
    try {
      const response = await fetch("/api/qatar-nia-gap-analysis")
      if (response.ok) {
        const data = await response.json()
        setGapAnalysis(data)
      }
    } catch (error) {
      console.error("Failed to fetch Qatar NIA gap analysis:", error)
      toast({
        title: "Error",
        description: "Failed to load Qatar NIA gap analysis",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold bg-gradient-to-r from-teal-600 via-blue-600 to-purple-600 bg-clip-text text-transparent">
            Qatar NIA Gap Analysis
          </h2>
          <p className="text-muted-foreground">
            Identify and prioritize gaps in Qatar NIA security controls implementation
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="border-border bg-card">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-teal-600">QAR {totalCost.toLocaleString()}</div>
      ...
      <Card className="gradient-card-primary">
        <CardHeader>
          <CardTitle className="bg-gradient-to-r from-teal-600 to-blue-600 bg-clip-text text-transparent">
            Qatar NIA Security Control Gaps
          </CardTitle>
          <CardDescription>Prioritized list of security control implementation gaps</CardDescription>
        </CardHeader>
        <CardContent>
          {/* Filters */}
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="flex items-center space-x-2 flex-1">
              ...
              <Input
                placeholder="Search by control name, ID, or responsible party..."
                value={searchTerm}
      ...
      <Card className="gradient-card-secondary">
        <CardHeader>
          ...
          <CardDescription>Summary of gaps across Qatar NIA security domains</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              return (
                <Card key={domain} className="border-purple-200/50">
                  ...
                  <CardContent className="space-y-2">
                    ...
                    <div className="flex justify-between text-xs">
                      <span className="text-teal-600">Investment:</span>
                      <span className="font-medium">QAR {domainCost.toLocaleString()}</span>
                    </div>
                    <Button
                      variant="outline"
                      size="sm"
