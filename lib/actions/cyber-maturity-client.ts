...
export async function deleteMaturityAssessment(id: number) {
  return apiFetch(`/api/cyber-maturity/maturity-assessments/${id}`, { method: "DELETE" });
}
...
export async function createGapAnalysis(payload: any) {
...
export async function deleteRemediationTracking(id: number) {
  return apiFetch(`/api/cyber-maturity/remediation-tracking/${id}`, { method: "DELETE" });
}
