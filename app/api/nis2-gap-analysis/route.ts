...
export const GET = withContext(async({ tenantDb }, request) => {
  try {
    const gapAnalysis = await tenantDb`
      SELECT *
      FROM nis2_gap_analysis
      ORDER BY priority_score DESC, domain, control_id
    `

    return NextResponse.json(gapAnalysis)
  } catch (error) {
    console.error("Error fetching NIS2 gap analysis:", error)
    return NextResponse.json({ error: "Failed to fetch NIS2 gap analysis" }, { status: 500 })
  }
});
...
export const POST = withContext(async({ tenantDb }, request) => {
  try {
    ...
    console.error("Error creating NIS2 gap analysis:", error)
    return NextResponse.json({ error: "Failed to create NIS2 gap analysis" }, { status: 500 })
  }
});
