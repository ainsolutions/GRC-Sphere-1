...
export const GET = withContext(async({ tenantDb }, request) => {
  try {
    const gapAnalysis = await tenantDb`
      SELECT *
      FROM sama_gap_analysis
      ORDER BY priority_score DESC, domain, control_id
    `

    return NextResponse.json(gapAnalysis)
  } catch (error) {
    console.error("Error fetching SAMA gap analysis:", error)
    return NextResponse.json({ error: "Failed to fetch SAMA gap analysis" }, { status: 500 })
  }
});
...
export const POST = withContext(async({ tenantDb }, request) => {
  try {
    ...
    console.error("Error creating SAMA gap analysis:", error)
    return NextResponse.json({ error: "Failed to create SAMA gap analysis" }, { status: 500 })
  }
});
