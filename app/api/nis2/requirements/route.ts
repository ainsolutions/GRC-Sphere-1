...
export const GET = withContext(async({ tenantDb }, request) => {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch NIS2 requirements" }, { status: 500 })
  }
});
...
export const POST = withContext(async({ tenantDb }, request) => {
  try {
    ...
    console.error("Error creating NIS2 requirement:", error)
    return NextResponse.json({ error: "Failed to create NIS2 requirement" }, { status: 500 })
  }
});
...
export const PUT = withContext(async({ tenantDb }, request) => {
  try {
    ...
    return NextResponse.json({ error: "Failed to update NIS2 requirement" }, { status: 500 })
  }
});
