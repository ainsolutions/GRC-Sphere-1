...
export const GET = withContext(async ({ tenantDb }: HttpSessionContext, request) => {
  try {
    const gapAnalysis = await tenantDb`
      SELECT *
      FROM dora_gap_analysis
      ORDER BY priority_score DESC, domain, control_id
    `

    return NextResponse.json(gapAnalysis)
  } catch (error) {
    console.error("Error fetching DORA gap analysis:", error)
    return NextResponse.json({ error: "Failed to fetch DORA gap analysis" }, { status: 500 })
  }
});
...
export const POST = withContext(async ({ tenantDb }: HttpSessionContext, request) => {
  try {
    ...
    console.error("Error creating DORA gap analysis:", error)
    return NextResponse.json({ error: "Failed to create DORA gap analysis" }, { status: 500 })
  }
});
